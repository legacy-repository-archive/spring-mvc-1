...
```
HTTP 파라미터 이름이 변수 이름과 같으면 `@RequestParam(name="xx")`에서       
name/value 속성을 생략한 `@RequestParam`만 기입 가능하다.         
...
파라미터 타입이 String , int , Integer 등의 단순 타입이면         
...
단 매우 주의 사항이 있으니 이를 기억하자.   
`/request-param?username=` : 빈문자로 에러 없이 통과
`/request-param` : 
